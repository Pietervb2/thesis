...
\begin{enumerate}
    ...
    \item The pipes are cylindrical and are completely filled with water.
    \item The fluid flow is one-directional. 
    \item The fluid velocity and temperature are assumed to be uniform across the pipe cross-section.
...
\begin{equation}\label{eq::DisPresQ}
R = f\frac{8 L}{\pi^2 D^5 \rho} |\dot{m}|\dot{m} + \rho_w g L \hat{z}
\end{equation}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.65\linewidth]{Thesis report/figures_thesis/}
...
However, in the case of low flow rate, these methods experience weak convergence. The method developed by Arsene et al. \cite{ARSENE}, which can handle lower flow rates and will therefore be used within the simulation models created in this study.
